...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    main {{ width:min(1560px,calc(100vw - 32px)); margin:auto; padding:12px 0 48px; }}
    header {{ display:grid; grid-template-columns:minmax(240px,1fr) auto minmax(240px,1fr); align-items:center; gap:16px; margin-bottom:16px; min-height:48px; position:sticky; top:0; z-index:20; padding:8px 0; background:var(--bg); }}
    ...
    nav {{ grid-column:2; justify-self:center; display:flex; flex-wrap:wrap; gap:6px; }}
    nav a {{ display:grid; place-items:center; min-height:34px; padding:7px 10px; border:1px solid var(--line); border-radius:5px; background:var(--surface); color:var(--ink); font-size:12px; font-weight:800; line-height:1; text-decoration:none; transition:border-color .18s ease,background .18s ease; }}
    ...
    .api-indicator.offline {{ color:#b5121b; border-color:#d26b72; }}
    .api-indicator.offline .api-indicator-dot {{ background:#d33b46; box-shadow:0 0 0 3px rgba(211,59,70,.18); }}
    ...
    .language-switch button.active {{ background:#171a1f; color:white; }}
    ...
    @media (max-width:760px) {{ main {{ width:min(100vw - 16px,1560px); padding-top:4px; }} header {{ grid-template-columns:1fr; align-items:stretch; gap:8px; }} .brand-lockup, nav, .topbar-actions {{ grid-column:1; }} nav {{ display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); width:100%; }} nav a {{ min-width:0; padding-inline:5px; text-align:center; }} .topbar-actions {{ display:grid; grid-template-columns:auto auto 34px; width:100%; justify-content:space-between; }} .api-indicator {{ max-width:150px; overflow:hidden; text-overflow:ellipsis; }} .panel {{ overflow-x:auto; }} .hero {{ padding:16px; }} }}
  </style>
</head>
<body><main>
  <header><a class="brand-lockup" href="index.html" data-nav="brand" aria-label="Race Energy Orchestrator Dashboard"><span class="brand-mark" aria-hidden="true"><i></i><i></i><i></i></span><span>Race Energy Orchestrator</span></a><nav><a href="index.html" data-nav="dashboard" data-i18n="navDashboard">Dashboard</a><a class="{'active' if title.startswith('Telemetry') else ''}" href="explorer.html" data-nav="telemetry" data-i18n="navTelemetry">Telemetry</a><a class="{'active' if title.startswith('Guide') else ''}" href="guide.html" data-nav="guide" data-i18n="navGuide">Guide</a></nav><div class="topbar-actions"><span class="api-indicator checking" id="reo-api-indicator"><i class="api-indicator-dot" aria-hidden="true"></i><span id="reo-api-indicator-text">KONTROL EDİLİYOR</span></span><div class="language-switch" aria-label="Language"><button id="lang-tr" class="active" type="button">TR</button><button id="lang-en" type="button">EN</button></div><button class="theme-toggle" id="reo-theme-toggle" type="button" aria-label="Toggle dark mode">◐</button></div></header>
  {body}
</main><script>
(() => {{
  const dictionary = {{
  ...
  let supportApiState = 'checking';
  const renderApiIndicator = () => {{
    const language = document.documentElement.lang === 'en' ? 'en' : 'tr';
    const labels = language === 'en'
      ? {{ checking:'CHECKING', live:'CONNECTED', offline:'OFFLINE' }}
      : {{ checking:'KONTROL EDİLİYOR', live:'BAĞLI', offline:'ÇEVRİMDIŞI' }};
    const indicator = document.getElementById('reo-api-indicator');
    if (indicator) indicator.className = `api-indicator ${{supportApiState}}`;
    const text = document.getElementById('reo-api-indicator-text');
    if (text) text.textContent = labels[supportApiState];
  }};
  ...
  fetch(`${{document.documentElement.dataset.apiBase}}/api/health`, {{ cache:'no-store' }})
    .then(response => {{ if (!response.ok) throw new Error('API unavailable'); supportApiState = 'live'; renderApiIndicator(); }})
    .catch(() => {{ supportApiState = 'offline'; renderApiIndicator(); }});
}})();
</script></body></html>"""
